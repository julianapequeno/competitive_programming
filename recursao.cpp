...
int qtdElementos3n1(int n)
{
	...
	if (n % 2 == 0)
		...
		return 1 + qtdElementos3n1(n * 3 + 1);
}

// retorna a soma dos dígitos de um número n. O número pode ser negativo.
int somaDigitos(int n)
{
	if (to_string(abs(n)).size() == 1)
		return n;
	else
	{
		n = stoi(to_string(abs(n)).substr(0, to_string(abs(n)).size()));
		return abs(stoi(to_string(abs(n)).substr(to_string(abs(n)).size() - 1))) + somaDigitos(stoi(to_string(abs(n)).substr(0, to_string(abs(n)).size() - 1)));
	}
}
...
bool saquePossivel(int n, int x, int a, int y, int b)
{
	...
	{
		if (x > 0 & a <= n & (n - a >= 0) & ((n % b != 0) || (n % a == 0)) & ((a > b) || y == 0 || (n % b != 0)))
		{
			return saquePossivel(n - a, x - 1, a, y, b);
		}
		else if (y > 0 & b <= n & (n - b >= 0))
		{
			return saquePossivel(n - b, x, a, y - 1, b);
		}
		else
		{
			return false;
